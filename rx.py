...
algo = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Container([
                html.H1("RX Insurance Genieee", style={'textAlign':'center', 'fontSize':85},className="display-3"
                       ),
                html.P(
    ...
    dbc.Row([
        dbc.Col([
            dbc.CardDeck(
                [
                    dbc.Card([                       
                        html.Div(html.Img(src=image1, style={'height':'100%', 'width':'50%'}),style={'textAlign': 'center'}),
    
                        #dbc.CardImg(src=image1, top=True),
                        dbc.CardBody([
                            
                            html.H4([html.Span("Step 1: Choose Med", id='tooltip-target1', style={"textDecoration": "underline", "cursor": "pointer"},className="card-title")],style={'textAlign':'center'}),
                    ...
                    dbc.Card([
                        html.Div(html.Img(src=image4, style={'height':'150%', 'width':'50%'}),style={'textAlign': 'center'}),

                        dbc.CardBody([
                            
                            html.H4([html.Span("Step 2: Choose Insurance", id='tooltip-target2', style={"textDecoration": "underline", "cursor": "pointer"},className="card-title")],style={'textAlign':'center'}),
                    ...
                    dbc.Card([
                        html.Div(html.Img(src=image3, style={'height':'100%', 'width':'50%'}),style={'textAlign': 'center'}),
    ...
    dbc.Card([                       
        ...
        dbc.CardBody([
            ...
            dbc.Tooltip("Spread is the net difference between what you are reimbursed for a medication and what it costs you to purchase.  In this tool, spread is defined per unit."
                ...
                "Note that in other dosage forms, such as solutions, eye drops, or inhalers, a unit may be defined by however that product is measured, such as an ml or gram.",target="spread"),
    ...
    html.Div([
        dcc.Tabs(id='tabs-example', value='tab-1', children=[                
            ...
            dcc.Tab(label='', style={"textDecoration": "underline", "cursor": "pointer"},id='first-tab', value='tab-1'),
            dcc.Tab(label='', style={"textDecoration": "underline", "cursor": "pointer"},id='second-tab', value='tab-2'),
            dcc.Tab(label='', style={"textDecoration": "underline", "cursor": "pointer"},id='third-tab', value='tab-3')
        ]),
        html.Div(id='tabs-example-content')
    ]),
    
    dbc.Tooltip("Results from the exact medication/strength you are researching. These are the most PRECISE results with regard to the medication and strength you have selected.",target="first-tab"),
    dbc.Tooltip("Results from the entire class of medications based on the medication you input (ie if you chose atorvastatin, this tab would show all of the statin medications and all of their strengths, including pravastatin, simvastatin, rosuvastatin, etc).  These are BROADER results, but would give you a WIDER range of optional reimbursement opportunities.",target="second-tab"),
    dbc.Tooltip("Results from the entire disease state based on the medication you input (ie if you chose atorvastatin, you would get ALL possible classes of medications used to treat cholesterol - statins, bile acid sequestrants, fish oils, etc).   These are the BROADEST possible results, but would give you the WIDEST range of optional reimbursement opportunities.",target="third-tab"),
...
data = dbc.Container([
        ...
        html.P('The data used in this tool is an aggregate of reimbursement data collected from prescription insurance claims.'
        ...
        html.P("The tool would be best used if a healthy number of independent pharmacies were able to submit their reimbursement data in realtime"
        ...
        dcc.Markdown('*The goal would be for the tool to function and view medication reimbursement in the same vein as the stock market.*'),
        dbc.Row(html.P()),
    
        dcc.Markdown('**Can you use machine learning for predictive analysis and build that into the tool?**'),
        html.P("The mechanisms by which the PBM function to price reimbursement seem to vary without any discernable pattern, though this is something that I am looking for trends on."),
        dbc.Row(html.P()),
               
        dcc.Markdown('**What can be improved upon in this tool?**'),
        html.P("One thing that will vary from pharmacy to pharmacy is acquisition cost of medication, which obviously can affect the spread.  As more pharmacies use the tool, there would need to be a way to keep track of this cost in order to properly calculate an accurate spread."),
        dbc.Row(html.P()),
...
def render_content(tab, plan, group, med):
    ...
    if tab == 'tab-1':
        ...
        heading = html.Div([
            ...
            dbc.Tooltip('Tables are pre-sorted according to the highest spread.  You also have the ability to sort by date as the highest spread NDC may be an older claim.',
                        target="heading1",),
        ])
    ...
    # TAB 2: CLASS OF MEDS
        ...
        fig.update_layout(title_text='All Spreads For All '+ med_class, title_x=0.5, yaxis_title="Spread Per Unit in Dollars")
